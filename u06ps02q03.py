...
def ordered_search(index, ranks, keyword):
    ...
    if keyword in index:
        for url in index[keyword]:
            rankings.append(ranks[url])
        ordered = ordered_list(rankings)
        ranked_search = []
        for e in ordered:
            for url in index[keyword]:
                if ranks[url] == e:
                    ranked_search.append(url)
        return ranked_search


def ordered_list(rankings):
    if len(rankings) == 0:
        return rankings
    if len(rankings) == 1:
        return rankings
    ...
    for rank in rankings:
        ...
        if rank < pivot:
            bigger.append(rank)
    return ordered_list(smaller) + [pivot] + ordered_list(bigger)
...
rank_list = [2,4,5,10,120,2,7,9]

print ordered_list(rank_list)
