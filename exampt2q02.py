...
def reciprocal_link(graph, end_links, start_links, k):
    if k == 0:
        return start_links
    for links in start_links:
        for element in graph[links]:
            end_links.append(element)
    return end_links + reciprocal_link(graph, [], end_links, k - 1)
...
def compute_ranks(graph, k):
    ...
    for i in range(0, numloops):
        ...
        for page in graph:
            newrank = (1 - d) / npages
            excluded = reciprocal_link(graph, [], page, k)
            for node in graph:
