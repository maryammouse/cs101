...
# TO DO:
# Define the rules
# Define what rules the rule number will assign
# Simple while-loop while generations is > 0 to keep editing the
# string until we're at the last generation.
...
def rule(n, stringy):
    ...
    for element in stringy:
        new_stringy = new_stringy + '0'
        original = original + '0'
    while k < len(stringy):
        if n == 1:
            if k == len(stringy) - 1:
                if stringy[k -1] == '.' and stringy[k] == '.' and stringy[0] == '.':
                    new_stringy = new_stringy[:k] + '.'
            else:
                if stringy[k-1] == '.' and stringy[k] == '.' and stringy[k + 1] == '.':
                    new_stringy = new_stringy[:k] + '.' + new_stringy[k + 1:]
        if n == 2:
            if k == len(stringy) - 1:
                if stringy[k -1] == '.' and stringy[k] == '.' and stringy[0] == 'x':
                    new_stringy = new_stringy[:k] + 'x'
            else:
                if stringy[k -1] == '.' and stringy[k] == '.' and stringy[k + 1] == 'x':
                    new_stringy = new_stringy[:k] + 'x' + new_stringy[k + 1:]
        if n == 4:
            if k == len(stringy) - 1:
                if stringy[k -1] == '.' and stringy[k] == 'x' and stringy[0] == '.':
                    new_stringy = new_stringy[:k] + 'x'
            else:
                if stringy[k-1] == '.' and stringy[k] == 'x' and stringy[k + 1] == '.':
                    new_stringy = new_stringy[:k] + 'x' + new_stringy[k + 1:]
        if n == 8:
            if k == len(stringy) - 1:
                if stringy[k - 1] == '.' and stringy[k] == 'x' and stringy[0] == 'x':
                    new_stringy = stringy[:k] + 'k'
            else:
                if stringy[k-1] == '.' and stringy[k] == 'x' and stringy[k + 1] == 'x':
                    new_stringy = stringy[:k] + 'k'

        k += 1
    if new_stringy == original:
        return stringy
    else:
        return new_stringy
...
print test[3]
new_test = test[:3] + 'y' + test[4:]
print new_test



def cellular_automaton(stringy, rule, generations):
    pass
