...
def hashtable_update(htable, key, value):
    ...
    hashtable_shortcut(htable, key, value, True, False)
...
def hashtable_lookup(htable, key):
    ...
    #        return entry[1]
    #return None
    return hashtable_shortcut(htable, key, None, False, True)

def hashtable_shortcut(htable, key, value, update, lookup):
    bucket = hashtable_get_bucket(htable, key)
    for entry in bucket:
        if entry[0] == key:
            if lookup == True:
